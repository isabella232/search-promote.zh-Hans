...
但是，在使用动态分页时，不会对核心搜索可以有效跟踪的彩块化数量设置限制。 您可以定义数百个动态彩块化，核心搜索可以从中返回给定搜索的“前`N`动态彩块化”，其中`N`通常是10-20或更少的较小值。 此方法无需插入属性 — 您现在可以为网站中的属性创建唯一的动态facet。

## 您应该让哪些方面变得动态？{#section_254EE034BCAD4250A5D09FBF6158C4A5}
...
## facet与meta标签字段{#section_2869E5FCDA8B431A87BC6E5573F2B0A0}的关系

彩块化构建在元标签字段的顶部。 meta标签字段是[!DNL Adobe Search&Promote]的低级核心搜索层功能。 另一方面，彩块化是GS（向导搜索）的一部分，GS是AdobeSearch&amp;Promote的高级表示层。 Facet拥有meta标签字段，但meta标签字段对facet一无所知。 在配置动态彩块化时，您首先添加彩块化，然后添加元标签字段，同时选中动态彩块化选项，以将标识彩块化设置为动态彩块化。

>[!NOTE]
>
>**[!UICONTROL Design > Navigation > Facets]**&#x200B;中没有“动态Facet”设置。 facet的“dynamic”是其基础“meta tag field”是动态的，如&#x200B;**[!UICONTROL Settings > Metadata > Definitions]**&#x200B;中所设置。
...
* [演示文稿模板标签](../c-appendices/c-templates.md#reference_F1BBF616BCEC4AD7B2548ECD3CA74C64)
...
1. 添加您的彩块化后，请确保您已将彩块化添加到新的用户定义的元标签字段。
...
1. 在[!DNL Edit Field]页面上，选中&#x200B;**[!UICONTROL Dynamic Facet]**。

   请参阅[添加新meta标记字段](../c-about-settings-menu/c-about-metadata-menu.md#task_6DF188C0FC7F4831A4444CA9AFA615E5)中的选项表。
1. 单击 **[!UICONTROL Save Changes]**.
1. 单击蓝框中的&#x200B;**重新生成分阶段网站索引**&#x200B;以快速重建分阶段网站索引。

   另请参阅[重新生成实时或分阶段网站的索引](../c-about-index-menu/c-about-regenerate-index.md#task_B28DE40C0E9A475ABCBCBC4FF993AACD)。
1. 确定要为给定搜索选择的动态彩块化数量。 通过执行以下任一操作即可完成此任务:

   * 创建具有任何所需条件的查询清除规则，执行`set`、`backend parameter`、`sp_sfvl_df_count`以值`X`的操作，其中`X`是搜索时要请求的所需数量的动态彩块化，然后单击&#x200B;**[!UICONTROL Add]**。
   ...
   另请参见表中的[后端搜索CGI参数](../c-appendices/c-cgiparameters.md#reference_582E85C3886740C98FE88CA9DF7918E8)第40行，以进一步说明`sp_sfvl_df_count`。

   * 添加搜索，将“custom” `sp_sfvl_df_count`参数设置为所需值，然后单击&#x200B;**[!UICONTROL Add]**。
...
1. 编辑相应的传输模板以输出核心搜索返回的动态彩块化。
   ...
   例如，假定传输模板名为`guided.tpl`。 在这种情况下，请在产品菜单上单击&#x200B;**[!UICONTROL Design > Templates]**。 在[!DNL Templates]页面上，找到表中的`guided.tpl`。 然后，单击名称最右侧的&#x200B;**[!UICONTROL Edit]**。 在“编辑”页面上，将以下代码块添加到`</facets>`末尾：JSON输出：
...
1. 编辑相应的演示文稿模板或模板以输出动态彩块化。
   ...
   例如，假设您有一个名为`sim.tmpl`的模板，用于输出模拟器中的内容。 要编辑该模板，请在产品菜单上单击&#x200B;**[!UICONTROL Design > Templates]**。 在[!DNL Templates]页面上，找到表中的`sim.tmpl`。 然后，单击名称最右侧的&#x200B;**[!UICONTROL Edit]**。 在“编辑”页面上，在模板的facet显示区域中添加以下内容：
   ...
   您还可以根据需要对其他演示模板进行类似修改，如`json.tmpl`。

   请确保在`guided-facet-rail`标记中为`gsname`指定`__dynamic_facets`。 此标记是预定义的facet边栏，保留用于输出为给定搜索返回的任何动态facet。

   您还可以选择通过&#x200B;**[!UICONTROL Rules > Business Rules]**&#x200B;和使用&#x200B;**[!UICONTROL Advanced Rule Builder]**&#x200B;编辑此特殊facet边栏，如下所示。
