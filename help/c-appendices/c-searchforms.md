...
搜索模板标签`<search-input-collections>`在搜索结果中生成集合列表框HTML，并自动选择在搜索中指定的集合。 如果要生成复选框，请使用`<search-input>`标记，而不要使用`<input>`标记，如下所示：
...
`<search-input>`标记输出`<input>`标记，如果在搜索中指定了集合，则该标记包括`checked`属性。

## 使用表单为{#reference_82CDDDA1E37042E4849EBF7EA05407C5}的帧

您可以配置框架集以与网站搜索／销售结合使用。

要进一步了解HTML框架和HTML框架集元素，请参阅以下URL:

[https://www.w3schools.com/html/html_frames.asp](https://www.w3schools.com/html/html_frames.asp)

如果您的站点使用帧，则可以为搜索结果链接指定目标帧。 默认目标为_self，它在当前框架或浏览器窗口中打开链接。 您可以指定站点特定的目标或浏览器保留的数据：

* _top（浏览器保留）结果在当前浏览器窗口中打开并替换所有当前帧。
* _blank（浏览器保留）结果在新浏览器窗口中打开。
* _parent(browser-reserved)结果在当前帧的父帧中打开。
* frame2（站点特定）结果在名为“frame2”的帧中打开。 您可以将任何框架的名称指定为值（例如，主框架或内容）。

如果您的站点不使用框架，则最好不要更改默认目标名。
...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p> <a href="#section_BAA8A502BB2243F8B5FF9783CDF2BFFD" type="section" format="dita" scope="local"> 正在将搜索表单代码添加到您的框架中……  </a> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>2 </p> </td> 
   <td colname="col02"> <p>为搜索结果页面设置目标框。 </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col02"> <p>设置从搜索结果页面创建的链接目标。 </p> </td> 
...
1. 将搜索表单代码粘贴到框架集中所需的框架中。
   ...
   在以下示例中，搜索表单代码会粘贴到导航框架中——屏幕左侧的窄小垂直框架。

   ![](assets/frames1.gif)

## 为搜索结果页面{#section_532CACB90888467093D95EACB64FDFA1}设置目标框架

如果如上所示将搜索表单代码放置到垂直导航框架中，则可以在较大的主框架中显示搜索结果。 在此示例中，您将主帧调用为“body”，并将其设置为目标帧。

![](assets/frames2.gif)

1. 要为结果页指定目标框，请通过更改搜索表单代码中的以下行（从以下代码），向表单添加目标和值：
...
如果搜索结果显示在“body”框架中，您可能也希望在“body”框架中打开链接。 由于这是同一帧，即默认设置的目标值`"_self"`，因此您无需进行任何更改。

您还可以为结果链接设置目标帧。 以下是您可以执行的操作的几个示例：

* 为搜索结果及其链接指定不同的框架，以便当每个单击的结果在单独的框架中打开时，搜索结果在其自己的框架中保持活动状态。
* 指定搜索结果将打开到新的空白窗口中，以便旧窗口在其原始内容中保持活动状态，同时保留搜索结果。
...
* `target="_top"` 在完整窗口中打开链接。如果文档已位于顶部，则默认情况下此函数类似于`"_self"`。 使用此选项可打破任意深度的框架嵌套。

例如，要设置`_blank`目标目标帧，可以采用以下方式编辑模板：

1. 在产品菜单中，单击&#x200B;**[!UICONTROL Design]** > **[!UICONTROL Templates]**。

1. 在[!DNL Staged Templates]页面的表中，单击目标目标帧的模板名称。
1. 找到`<search-link>`标记。 默认的`<search-link>`标记应类似于以下内容：

   `<search-link><search-title length=100></search-link>`

1. 将帧目标添加到`<search-link>`标记。 在以上示例中，输入`target="_blank"`。 确保在目标值周围加下划线和引号。
...
## 编辑导航框架页面以防止对它们进行索引{#section_C62E5F0EE1294D5EBD97E123E54433FC}

通常，您希望排除导航框架，使其不能与搜索结果建立索引。 要实现此功能，可向这些页面添加`noindex` meta标记。

1. 打开导航框架的HTML页面源。
1. 在HTML的`<head>`部分中添加以下meta标签：
...
如果为安全内容编制索引，则可以从安全搜索Web服务器提供搜索结果。 将搜索表单操作属性中的URL更改为：action=&quot;https://search.atomz.com/search/&quot;。

>[!NOTE]
>
>某些HTML编辑器无法从其他应用程序粘贴HTML代码。 如果HTML代码以文本形式显示在网页上，则将搜索代码复制并粘贴到简单文本编辑器（如Windows上的记事本或Mac上的简单文本）中，然后再次从简单文本编辑器复制并粘贴到HTML编辑器中。

搜索参数用于高级搜索表单代码中，以创建单选按钮、复选框和列表框，客户可以使用这些按钮、复选框和搜索框来自定义单个搜索。 客户可以指定显示的搜索结果数，例如，日期范围，或是通过高级搜索表单上显示的选项全部指定显示的摘要。
...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col4"> <p>启用或禁用高级搜索选项。 例如，您可以在主页上放置一个标准的搜索表单，其中包含指向包含高级表单的第二页的链接。 在这种情况下，您应将标准表单的副本放在<span class="codeph"> &lt;search-if-not-advanced&gt;中……&lt;/search-if-not-advanced&gt; </span>模板标记。 </p> <p>在显示搜索结果时，从标准表单执行搜索的客户将看到标准搜索表单。 在高级搜索表单屏幕上，您将<span class="codeph"> &lt;input type=hidden name="sp_advanced" value=1&gt; </span>标记与其他高级表单选项一起使用。 </p> <p>您还可以在&lt;search-if-advanced&gt;... &lt;/search-if-advanced&gt;模板标记中包含高级搜索表单的副本。 在显示搜索结果时，从高级搜索表单执行搜索的客户会看到高级搜索表单。 </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col4"> <p>允许客户指定“任何单词”、“所有单词”或“确切的短语”必须存在，文档才能匹配。 当指定<span class="codeph"> sp_p </span>参数时，客户无需在搜索查询中使用“+”或“-”，或同时使用这两个参数。 </p> <p> 如果忽略<span class="codeph"> sp_p </span>参数，或将其设置为“”或“any”，则客户仍可使用“+”和“-”说明符。 如果将<span class="codeph"> sp_p </span>参数设置为“all”或“phrase”，则指定的“+”和“-”将被忽略。 </p> <p>您可以进一步了解在搜索中使用“+”和“-”。 </p> <p>请参阅<a href="../c-about-settings-menu/c-about-searching-menu.md#concept_207105CF26B1448F8A3D223787C56AB8" type="concept" format="dita" scope="local">关于搜索</a>。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col2"> <p> 相似音匹配 </p> </td> 
   ...
   <td colname="col4"> <p>允许客户启用或禁用类似声音匹配。 相似声音匹配允许拼写错误的搜索查询匹配文档中“相似声音”的词。 </p> <p>当<span class="codeph"> sp_w_control </span>参数设置为1且<span class="codeph"> sp_w </span>参数设置为“alike”时，将选中生成的复选框，默认情况下启用类似声匹配。 </p> <p>如果<span class="codeph"> sp_w </span>参数设置为“”，则不选中该复选框。 </p> <p>如果在最近的索引操作中未启用相似声音匹配，则无法实现相似声音匹配，并忽略<span class="codeph"> sp_w </span>参数。 要启用类似声音匹配，请在产品菜单上单击<span class="uicontrol">语言学</span> &gt; <span class="uicontrol">单词和语言</span> &gt; <span class="uicontrol">类似声音匹配</span>。 </p> <p>您还可以通过以下方式分配参数<span class="codeph"> sp_w </span>和<span class="codeph"> sp_w_control </span>: </p> <p> <code class="syntax html"> &lt;!--&nbsp;Checkbox&nbsp;disables&nbsp;sound-alike&nbsp;matching&nbsp;--&gt; 
      &lt;input&nbsp;type=hidden&nbsp;name="sp_w_control"&nbsp;value=0&gt; 
      &lt;input&nbsp;type=checkbox&nbsp;name="sp_w"&nbsp;value="exact"&gt; 
      No&nbsp;sound-alike&nbsp;matching </code> </p> <p>在这种情况下，当<span class="codeph"> sp_w_control </span>参数设置为0且<span class="codeph"> sp_w </span>参数设置为“exact,”默认情况下禁用类似声音匹配。 如果将<span class="codeph"> sp_w </span>参数设置为“”，则启用类似声音匹配。 </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col4"> <p><span class="codeph"> sp_d </span>参数指定要执行的自定义数据范围或要执行的特定日期范围。 </p> <p>在默认的高级搜索表单上，此选项以单选按钮组的形式显示，其下拉列表为“custom”日期范围，该日期范围由<span class="codeph"> sp_date_range </span>参数生成。 它还包括一组“特定”开始和结束日期，这些和结束日期由<span class="codeph"> sp_开始日</span>、<span class="codeph"> sp_开始月</span>、<span class="codeph"> sp_开始年</span>、<span class="codeph"> sp_end_day </span>、<span class="codeph"> sp_end_month &lt;a9生成/&gt;和<span class="codeph"> sp_end_year </span>参数。</span> </p> <p>“自定义”日期范围是要搜索的已命名日期范围。 例如，“随时”、“今天”、“去年”等。 </p> <p>“特定”日期范围包括开始日期和结束日期。 例如，从“2009年9月8日至2011年10月18日”。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col2"> <p>日期范围匹配：自定日期范围 </p> </td> 
   ...
   <td colname="col3"> <p> <code class="syntax html"> &lt;!--Selection&nbsp;list&nbsp;for&nbsp;custom&nbsp;date&nbsp;range.--&gt; 
      ...
      &lt;/select&gt; </code> </p> </td> 
   <td colname="col4"> <p><span class="codeph"> sp_date_range </span>参数用于创建“自定义”日期范围。 例如，“随时”、“今天”、“去年”等。 </p> <p>大于或等于零的值指定今天之前要搜索的天数。 例如，值0指定“今天”，值1指定“今天和昨天”，值30指定“最近30天内”，依此类推。 小于零的值指定自定义范围，如下所示： </p> <p> 
     <ul id="ul_E65DDE33883F441F9730F315E485AD98"> 
      ...
      <li id="li_F4C3A8658428418A8A06FBAAB4733C68"> <p>-3 =“上周”，从星期日到本周前一周的星期六搜索。 </p> </li> 
      <li id="li_DF2D0B043A4E4DE9BE8D82E69A76E793"> <p>-4 =“本月”，搜索日期在当月内。 </p> </li> 
      <li id="li_76BC4C2CED574E2A81448158828BFF1B"> <p>-5 =“上个月”，搜索日期在当前月份之前的一个月内。 </p> </li> 
      <li id="li_17FF849384FB46D58AF6FF1D3BC408C8"> <p>-6 =“今年”，搜索日期在当年。 </p> </li> 
      <li id="li_E2B8B4DFF3914BBDB86D0EB77F52B305"> <p>-7 =“去年”，其搜索日期在当前年度之前的一年中。 </p> </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col3"> </td> 
   <td colname="col4"> <p>这三个数值指定要搜索的特定日期范围的开始日期。 请确保指定所有三个值，因为忽略部分指定的日期。 </p> <p>只指定开始日期、结束日期或同时指定开始日期和结束日期是合法的。 如果仅指定开始日期，则搜索包括日期在开始日期或之后的匹配文档。 如果仅指定结束日期，则搜索将包括结束日期或结束日期之前的匹配文档。 如果同时指定了开始日期和结束日期，则搜索将包括从开始日期到结束日期的匹配文档。 </p> <p>所有日期都相对于格林威治标准时间进行搜索。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col2"> <p> 日期范围匹配：结束日期 </p> </td> 
   <td colname="col1"> <p> <span class="codeph"> sp_end_day、sp_end_month、sp_end_year  </span> </p> <p> </p> </td> 
   <td colname="col3"> </td> 
   <td colname="col4"> <p>这三个数值指定要搜索的特定日期范围的结束日期。 请确保指定所有三个值，因为忽略部分指定的日期。 </p> <p>只指定开始日期、结束日期或同时指定开始和结束日期是合法的。 如果仅指定开始日期，则搜索包括日期在开始日期或之后的匹配文档。 如果仅指定结束日期，则搜索将包括结束日期或结束日期之前的匹配文档。 如果同时指定开始和结束日期，则搜索将包括从开始日期到结束日期的匹配文档。 </p> <p>所有日期都相对于格林威治标准时间进行搜索。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col2"> <p>在搜索字段中 </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col4"> <p>允许客户选择在每个搜索结果页面上显示的搜索结果数。 </p> <p>表单中可以有所需的选择，也可以选择最少。 确保“value=”值与显示的值匹配。 </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col4"> <p>允许客户选择是否显示每个匹配项的摘要文本。 </p> <p>如果要显示摘要，请将值设置为1。 如果要隐藏摘要，请将值设置为0。 您还可以将参数与一组单选按钮一起使用，如下例所示： </p> <p> <code class="syntax html"> &lt;!--&nbsp;Show&nbsp;or&nbsp;hide&nbsp;summaries&nbsp;in&nbsp;search&nbsp;results&nbsp;--&gt; 
  ...
  <tr> 
   ...
   <td colname="col3"> <p> <code class="syntax html"> &lt;!--&nbsp;Sort&nbsp;results&nbsp;by&nbsp;relevance&nbsp;or&nbsp;by&nbsp;date&nbsp;--&gt; 
      ...
      &lt;/select&gt; </code> </p> </td> 
   <td colname="col4"> <p>允许客户选择结果是按相关性还是日期顺序列出。 </p> <p>当值设置为1时，结果将从最近更改的文档列出到最近更改的文档。 当该值设置为0时，结果将从最相关到最不相关的列出。 您还可以将此参数与单选按钮一起使用，如以下示例所示： </p> <p> <code class="syntax html"> &lt;!--&nbsp;Sort&nbsp;results&nbsp;by&nbsp;relevance&nbsp;or&nbsp;by&nbsp;date&nbsp;--&gt; 
...
如果使用此代码，请记住将`sp99999999`的`sp_a`值替换为实际的帐号。
...
您可以向模板添加高级搜索表单HTML代码，使任何参数的默认选择与之前的搜索相同。

换言之，如果客户单击&#x200B;**[!UICONTROL Exact phrase]**&#x200B;单选按钮，则可确保在显示搜索结果时默认选择单选按钮。

通过从标准HTML标签中删除所有“已选”或“已选”说明符，然后替换以下HTML标签，即可实现此功能：
