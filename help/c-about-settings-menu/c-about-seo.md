...
您可以将每个此类元素定义为一段起始文本，后跟一列表字。 单词列表可以包括：

* 在选定时间段（例如，“上个月”）内，您网站上的常用搜索短语，但须遵守您的自定义排除。
* 页面所来搜索的一个或多个字段的值集。
* 您在列表中定义的静态词语和短语

人们用于SEO的最常见页面元素是页面标题、“关键字”和“描述”元标记。 您可以为这三个页面元素定义设置。 此外，您还可以为三种类型的页面分别定义以下三种设置：搜索结果页面、浏览页面和项目详细信息页面。

在保存或预览SEO设置时，会生成搜索（传输）模板的小段，这些小段可以包含在具有`<search-include>`模板标记的其他搜索模板中。 例如，您可以将搜索结果页面标题字段包含到包含以下内容的其他模板中：

```
<search-include file="seo/seo_search_title.tpl" />
```

SEO字段的`<search-include>`标记的`file`属性的九个可能值如下：
...
首先，按如上所述使用`<search-include>`将所需字段包括在`<general>`元素内的XML搜索模板中。

然后，将每个`<search-include>`标记都用`<general-field>`和`</general-field>`标记环绕，在`name`属性中提供字段名称，如下例所示：
...
然后，在演示文稿模板中，您可以使用`<guided-general-field>`标记在需要时插入命名字段，如下例所示：
...
总之，您可以定义可在网页中使用的九个不同的SEO字段。 它们包括：

* 搜索结果页面 — 标题、说明和关键字
* 浏览页面 — 标题、说明和关键字
* 项目详细信息页面 — 标题、说明和关键字

所有9个字段都使用相似的设置进行定义。 事实上，九个字段的名称（如“搜索结果页面”中的“标题”字段）只是关于您可能如何使用这些字段的建议。 您可以在演示文稿模板和搜索模板中的任何上下文中使用任何字段。

另请参阅[关于模板](../c-about-design-menu/c-about-templates.md#concept_06EB481B14864E18A8AE2BCD1D6EF0B5)。

另请参阅[演示文稿模板标签](../c-appendices/c-templates.md#reference_F1BBF616BCEC4AD7B2548ECD3CA74C64)。
...
您可以配置包含在页面标题、说明和关键字中的搜索结果词和短语的列表。

**配置搜索结果单词列表**

1. 在产品菜单上，单击&#x200B;**[!UICONTROL Settings]** > **[!UICONTROL SEO]** > **[!UICONTROL Search Result Pages]**。
1. 在[!DNL SEO Browse Pages Word List]页面中，在相应的[!DNL Title]、[!DNL Description]和[!DNL Keywords]组中设置所需的选项。
   ...
   <table> 
    ...
    <tbody> 
      ...
      <td colname="col2"> <p>要在单词“列表”前显示的文本。 </p> <p>例如，您可能希望“关键字”列表以“品牌”一词开头。 </p> </td> 
      </tr> 
      <tr> 
      ...
      <td colname="col2"> <p>列表要添加到搜索结果页面标题、浏览页面标题或项目详细信息页面标题的词。 </p> <p>单击<b>编辑</b>将单词添加到列表。 </p> <p>每行可添加一个词或短语。 完成后，单击<b>保存更改</b>，然后关闭页面以返回到主SEO页。 </p> </td> 
      </tr> 
      <tr> 
      <td colname="col1"> <p>删除这些单词和短语（不包括的字段值） </p> </td> 
      <td colname="col2"> <p>列表要从搜索结果页面标题、浏览页面标题或项目详细信息页面标题中删除的词。 </p> <p>单击<b>编辑</b>将单词添加到删除列表。 </p> <p>每行可添加一个词或短语。 完成后，单击<b>保存更改</b>，然后关闭页面以返回到主SEO页。 </p> </td> 
      </tr> 
    </tbody> 
    </table>
...
您可以配置浏览页面中包含在页面标题、说明和关键字中的字词和短语的列表。

**配置浏览页面单词列表**

1. 在产品菜单上，单击&#x200B;**[!UICONTROL Settings]** > **[!UICONTROL SEO]** > **[!UICONTROL Browse Pages]**。
1. 在[!DNL SEO Browse Pages Word List]页面中，在相应的[!DNL Title]、[!DNL Description]和[!DNL Keywords]组中设置所需的选项。
...
您可以配置页面标题、说明和关键字中包含的项目详细信息词和短语的列表。

**配置项目详细信息字列表**

1. 在产品菜单上，单击&#x200B;**[!UICONTROL Settings]** > **[!UICONTROL SEO]** > **[!UICONTROL Item Detail Pages]**。
1. 在[!DNL SEO Item Detail Word List]页面中，在相应的[!DNL Title]、[!DNL Description]和[!DNL Keywords]组中设置所需的选项。
...
SEO字段可包含包含的字段值，因此搜索结果可能取决于您指定的搜索查询。
...
1.  单击&#x200B;**搜索**。
   ...
   生成的“标题”、“说明”和“关键字”字段显示根据您刚输入的搜索查询插入到页面中的内容。
1. 单击&#x200B;**关闭**&#x200B;以返回主SEO页。
