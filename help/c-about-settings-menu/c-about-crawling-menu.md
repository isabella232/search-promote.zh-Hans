...
为使上述两个蒙版示例按预期工作，将首先列出包含蒙版，如下所示：
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>用于包括和排除蒙版。 </p> <p>搜索机器人通常在检查日期蒙版之前下载并分析每个文件。 发生此行为是因为某些文件类型可以在文件本身中指定日期。 例如，HTML文档可以包含设置文件日期的meta标签。 </p> <p>如果要根据文件的日期排除许多文件，并且您不希望在服务器上加载不必要的文件，则可以使用 
     ...
     <userinput>
       server-date 
     </userinput> 指定不从服务器下载被排除的文档。 这意味着，您的文档的索引创建时间更短，服务器上的负载也更小。 如果显示 
     <userinput>
       server-date 
...
在上面的列表中，使用第一个密码，它包含与服务器的身份验证请求相匹配的领域或URL。 例如，即使使用 `https://www.mysite.com/path1/path2/index.html` 中的文 `Realm3`件， `name2``password2` 也是因为用URL定义的密码列在用领域定义的密码之上。
...
该页 [!DNL Index Connector] 面会显示以下信息：
...
1. 在页 [!DNL Index Connector Add] 面上，设置所需的连接器选项。 可用的选项取决于您选 **[!UICONTROL Type]** 择的选项。
   ...
   <table> 
    ...
    <tbody> 
      ...
      <li id="li_0AF92270AE9F4BA8B2C7EE41FABC0F34"> <span class="uicontrol"> 标记 </span> <p>指定解析的XML数据的XPath表示形式。 使用上面的Adobe XML文档示例，在选项Itemtag下，可以使用以下语法映射它： </p> <p> <code> /record/@displayurl&nbsp;-&gt;&nbsp;page-url 
      /record/metadata/meta[@name='title']/@content&nbsp;-&gt;&nbsp;title 
      /record/metadata/meta[@name='description']/@content&nbsp;-&gt;&nbsp;desc 
      /record/metadata/meta[@name='description']/@content&nbsp;-&gt;&nbsp;body </code> </p> <p>以上语法的转换如下： </p> <p> 
      <ul id="ul_6400EBD08D424EADA1612FE4F7EFB640"> 
      <li id="li_9958F9B40D42434195597DBA9F2AF28F"> <code> /record/@displayurl&amp;nbsp;-&gt;&amp;nbsp;page-url </code> <p>记录 <span class="codeph"> 元素 </span> 的displayurl属性 <span class="codeph"> 映射 </span> 到元数据字段 <span class="codeph"> page-url </span>。 </p> </li> 
      ...
      <li id="li_E741CA59197D462EB2946EDE874AFDC8"> <code> /record/metadata/meta[@name='description']/@content&amp;nbsp;-&gt;&amp;nbsp;desc </code> <p>包含 <span class="codeph"> 在元数据元素内的任何元元素的内容，包含在记录元素内的内容，该记录元素的名称属性是 </span><span class="codeph"></span><span class="codeph"></span><span class="codeph"></span><span class="codeph"></span><span class="codeph"></span>记录元素的描述，其名称属性是映射到元数据字段的映射。 </p> </li> 
      <li id="li_E35EAE3D284D46D485D9064D7BB6AB13"> <code> /record/metadata/meta[@name='description']/@content&amp;nbsp;-&gt;&amp;nbsp;body </code> <p>包含在元 <span class="codeph"> 数据元元素内的任何元元素的内容，包含在记录元素 </span> ，其名称是对元数据字段主体的映射的 <span class="codeph"></span><span class="codeph"></span><span class="codeph"></span><span class="codeph"></span><span class="codeph"></span>记录元素，其名称属性是对元数据字段主体的映射的描述。 </p> </li> 
      ...
      <li id="li_DEA24003E97E406DA2510C43CCFDC70E"> <span class="uicontrol"> 主要密钥？ </span> <p>只有一个映射定义被标识为主键。 此字段将成为将此文档添加到索引时显示的唯一引用。 此值用于索引中文档的URL。 </p> <p>主键 <span class="uicontrol"> 值在索引连 </span> 接器配置所表示的所有文档中必须是唯一的——遇到的任何重复项都将被忽略。 如果您的源文档不包含用作主键的唯一值，但两个或两个以上字段结合在一起可形成唯一标识符 <span class="uicontrol"> ，则可以通过将多个标签定义与分隔值的垂直条 </span>(“|”)组合来定义 <i></i><span class="uicontrol"></span><span class="uicontrol"></span> “主键”。 </p> </li> 
      <li id="li_DEA24003E97E406DA2510C43CCFDC81F"> <span class="uicontrol"> 删除HTML? </span> <p>选中此选项后，将删除在此字段的数据中找到的任何HTML标记。 </p> </li> 
      <li id="li_5E829D1D0DBD4BB7AAB5DB983053D248"> <span class="uicontrol"> 是否用于删除？ </span> <p>仅在增量索引操作期间使用。 与此XPath模式匹配的记录标识要删除的项。 每个 <span class="uicontrol"> 此类记 </span> 录的主键值用于构造“删除”请求，就像删除文件路径一样。 </p> <p> <b>注意</b>:默认情况下不启用此功能。 请联系技术支持以激活该功能供您使用。 </p> </li> 
      ...
      <li id="li_537F24C6B2AB435CB7C14117663D7B3F"> HTTP <p>如有必要，您可以输入正确的身份验证凭据以访问HTTP服务器。 </p> </li> 
      <li id="li_8C13C93C52364FFA8B9B18830CDB223C"> HTTPS <p>如有必要，您可以输入正确的身份验证凭据以访问HTTPS服务器。 </p> </li> 
      ...
      </ul> </p> <p> <b>注意</b>:仅当在“主机地址”和／或“文件路径”字段中指定信息时，才使用“协议”设置。 单个XML文档会根据其URL规范使用HTTP或HTTPS进行下载。 </p> </td> 
      </tr> 
      <tr> 
      <td colname="col1"> <p>Itemtag </p> </td> 
      ...
      <li id="li_0FA2502869BA40DC93D790B79E15A9D2"> <span class="uicontrol"> 标记 </span> <p>指定解析的XML数据的XPath表示形式。 使用上面的Adobe XML文档示例，在选项Itemtag下，可以使用以下语法映射它： </p> <p> <code> /record/@displayurl&nbsp;-&gt;&nbsp;page-url 
        /record/metadata/meta[@name='title']/@content&nbsp;-&gt;&nbsp;title 
        /record/metadata/meta[@name='description']/@content&nbsp;-&gt;&nbsp;desc 
        /record/metadata/meta[@name='description']/@content&nbsp;-&gt;&nbsp;body </code> </p> <p>以上语法的转换如下： </p> <p> 
      <ul id="ul_F8C536E6E54546D9AA5B22B879C0AF39"> 
      <li id="li_78A35DFFF1B4496CAC6EDC7B1E991F29"> <code> /record/@displayurl&amp;nbsp;-&gt;&amp;nbsp;page-url </code> <p>记录 <span class="codeph"> 元素 </span> 的displayurl属性 <span class="codeph"> 映射 </span> 到元数据字段 <span class="codeph"> page-url </span>。 </p> </li> 
      <li id="li_FA7DF3D1942248B98660F3D0C82F4563"> <code> /record/metadata/meta[@name='title']/@content&amp;nbsp;-&gt;&amp;nbsp;title </code> <p>该内 <span class="codeph"> 容包含在元数据元素内的任何元元 </span> 素的内容，该元数据元素内包含记录元素 <span class="codeph"> ，其名称属性为元数据字段标题的映射的 </span><span class="codeph"></span><span class="codeph"></span><span class="codeph"></span><span class="codeph"></span>记录元素，其名称属性为元数据字段标题。 </p> </li> 
      <li id="li_D8000A116FF84DE59ED19C656DDD3BC1"> <code> /record/metadata/meta[@name='description']/@content&amp;nbsp;-&gt;&amp;nbsp;desc </code> <p>包含 <span class="codeph"> 在元数据元素内的任何元元素的内容，包含在记录元素内的内容，该记录元素的名称属性是 </span><span class="codeph"></span><span class="codeph"></span><span class="codeph"></span><span class="codeph"></span><span class="codeph"></span>记录元素的描述，其名称属性是映射到元数据字段的映射。 </p> </li> 
      <li id="li_7FA6A53DFD3D42A98B7BA17CC29DDB81"> <code> /record/metadata/meta[@name='description']/@content&amp;nbsp;-&gt;&amp;nbsp;body </code> <p>包含在元 <span class="codeph"> 数据元元素内的任何元元素的内容，包含在记录元素 </span> ，其名称是对元数据字段主体的映射的 <span class="codeph"></span><span class="codeph"></span><span class="codeph"></span><span class="codeph"></span><span class="codeph"></span>记录元素，其名称属性是对元数据字段主体的映射的描述。 </p> </li> 
      </ul> </p> <p>XPath是一个相对复杂的表示法。 有关更多信息，请访问以下位置： </p> <p>请参阅 <a href="https://www.w3schools.com/xpath/" scope="external" format="html"> https://www.w3schools.com/xpath/ </a> </p> </li> 
      ...
      <li id="li_9F435EFB3EC74B409EC82A851824610G"> <span class="uicontrol"> 删除HTML? </span> <p>选中此选项后，将删除在此字段的数据中找到的任何HTML标记。 </p> </li> 
...
1. （可选）单 **[!UICONTROL Preview]** 击以测试配置的实际工作情况。 此功能仅适用于文本和源类型。
1. 单 **[!UICONTROL Add]** 击以将配置添加到页 [!DNL Index Connector Definitions] 面和页面 [!DNL Index Connector Configurations] 的下拉列表 [!DNL URL Entrypoints] 中。
...
1. 在产品菜单中，单击 **[!UICONTROL Settings]** > **[!UICONTROL Crawling]** > **[!UICONTROL Index Connector]**。
1. 在页 [!DNL Index Connector] 面的列标题下，单 [!DNL Actions] 击“索引连接器”定 **[!UICONTROL Edit]** 义名称，您要更改其设置。
1. 在页 [!DNL Index Connector Edit] 面上，设置所需的选项。
...
在将索引连接器定义添加到页面 [!DNL Index Connector Definitions] 后，您无法更改其类型设置。 相反，您必须删除定义，然后添加新定义。

**查看索引连接器定义的设置**

1. 在产品菜单中，单击 **[!UICONTROL Settings]** > **[!UICONTROL Crawling]** > **[!UICONTROL Index Connector]**。
1. 在页 [!DNL Index Connector] 面的列标题下，单 [!DNL Actions] 击“索引连接器”定义 **[!UICONTROL Edit]** 名称，该名称的设置要查看或编辑。

## 复制索引连接器定义 {#task_3AD55DF07FC44A748D0EFDAB7B35699B}

您可以复制现有索引连接器定义，以用作要创建的新索引连接器的基础。

在复制索引连接器定义时，默认情况下，复制的定义处于禁用状态。 要启用或“打开”定义，您必须从页面中编辑该定 [!DNL Index Connector Edit] 义，然后选择 **[!UICONTROL Enable]**。
...
1. 在产品菜单中，单击 **[!UICONTROL Settings]** > **[!UICONTROL Crawling]** > **[!UICONTROL Index Connector]**。
1. 在页 [!DNL Index Connector] 面的列标题下，单 [!DNL Actions] 击以查找要复制其 **[!UICONTROL Copy]** 设置的索引连接器定义名称。
...
重命名定义后，选中 **[!UICONTROL Settings]** > **[!UICONTROL Crawling]** > **[!UICONTROL URL Entrypoints]**。 您需要确保新定义名称反映在页面的下拉列表中 [!DNL URL Entrypoints] 。
...
1. 在产品菜单中，单击 **[!UICONTROL Settings]** > **[!UICONTROL Crawling]** > **[!UICONTROL Index Connector]**。
1. 在页 [!DNL Index Connector] 面的列标题下，单 [!DNL Actions] 击“索引连接器 **[!UICONTROL Rename]** ”定义名称，以更改该名称。
1. 在页 [!DNL Index Connector Rename] 面上，在字段中输入定义的新名 [!DNL Name] 称。
1. 单击 **[!UICONTROL Rename]**.
1. 单击 **[!UICONTROL Settings]** > **[!UICONTROL Crawling]** > **[!UICONTROL URL Entrypoints]**. 如果列表中存在以前的索引连接器名称，则删除该名称，然后添加新重命名的条目。
...
1. 在产品菜单中，单击 **[!UICONTROL Settings]** > **[!UICONTROL Crawling]** > **[!UICONTROL Index Connector]**。
1. 在页 [!DNL Index Connector Definitions] 面的列标题下，单 [!DNL Actions] 击以获取要删除的 **[!UICONTROL Delete]** 索引连接器定义名称。
