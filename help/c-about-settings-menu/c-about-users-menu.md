---
description: 使用“用户”菜单视图和添加用户、视图和添加角色，或更改角色成员身份。 您必须是具有管理员权限的网站搜索／推销用户，才能执行“用户”菜单上的任何任务。
seo-description: 使用“用户”菜单视图和添加用户、视图和添加角色，或更改角色成员身份。 您必须是具有管理员权限的网站搜索／推销用户，才能执行“用户”菜单上的任何任务。
seo-title: 关于“用户”菜单
solution: Target
subtopic: Users
title: 关于“用户”菜单
...
使用“用户”菜单视图和添加用户、视图和添加角色，或更改角色成员身份。 您必须是具有管理员权限的网站搜索／推销用户，才能执行“用户”菜单上的任何任务。
...
1. （可选）单击[!DNL View Users]页面上的超链接角色名称。 此时将打开[!DNL Role Membership]页面，您可以在其中将用户分配到角色，也可以从角色中取消分配用户。
...
您可以使用[!DNL Add Users]页面向网站搜索／推销添加新帐户用户。

只需要新用户的电子邮件地址。 将新用户添加到帐户时，会向新用户发送帐户信息。

默认情况下，新用户不会分配为用户管理员。 用户管理员可以定义和编辑角色，并删除其他用户。 您可以从[!DNL Add Users]页面选择将新用户设为用户管理员。 或者，您也可以使用“视图用户”页面使新用户成为用户管理员。

请参阅[查看帐户用户](../c-about-settings-menu/c-about-users-menu.md#task_FDDF30EE23C548DF8CFBB2FB2605303C)。

您指定的电子邮件地址只能包含ASCII字符。 使用标准字母(a..z)字符或数字(0.9)字符，只有一个`@`字符，用于将用户名与域分开。 字符`_`、`+`、`-`、`!`、`#`、`$`、`'`、`%`、`&`、`*`、`=`、`?`、`^`、还允许a14/>和`}`。 `{``.`不要将电子邮件地址开始为`.`

如果新用户不是Adobe客户，系统会提示您为该用户创建客户登录名。 将向新用户发送登录口令和确认。 新用户首次登录时，会填写客户用户档案。

您可以选择单击[!DNL Add Users]页面上的超链接角色名称。 此时将打开[!DNL Role Membership]页面，您可以在其中将用户分配到角色，也可以从角色中取消分配用户。
...
## 查看帐户{#task_4EAE1D018F384691A083AD51E5CE58DC}的角色

您可以使用[!DNL View Roles]页面显示当前登录帐户当前存在的所有角色。
...
1. （可选）执行下列任一操作：
   ...
   * 在表的[!DNL Role Name]列标题下，单击超链接角色名称。 此时将打开[!DNL Role Membership]页面，您可以在其中将用户分配到角色，也可以从角色中取消分配用户。

      请参阅[配置角色成员关系](../c-about-settings-menu/c-about-users-menu.md#task_DAC596AAFFCE4EF0BE68CEEF7E365414)。

   * 在表的[!DNL Edit]列标题下，单击铅笔图标。 此时将打开[!DNL Edit Role]页面，您可以在其中重命名角色、更改说明等。
...
您可以重命名现有角色，更改其描述，并只选择要授予角色访问权限的用户界面区域。
...
1. （可选）执行下列任一操作：
   ...
   * 在[!DNL Role Name]文本字段中，根据需要键入新名称。 *表示该角色需要此字段。
   * 在[!DNL Description]文本字段中，根据需要键入角色的新描述。
   * 在组框中，选中或取消选中您希望角色访问或阻止的功能。

1. 单击 **[!UICONTROL Save Changes]**.

## 向帐户{#task_E148D02275DE4F899BA79736A29895AB}添加新角色

您可以使用[!DNL Add Roles]页面，更轻松、快速地分配帐户用户的权限。

例如，您可以单独授予您的公共关系部门的每个成员访问网站搜索／销售任务。 但是，将这些用户添加到“PR”角色，然后将任务分配给整个角色会效率更高。

每个角色名称必须唯一。 可以使用字母数字字符和常用符号，包括虚线`"-"`、下划线`"_"`和句点`"."`。 名称不能以下划线或句点开头。

在表中的[!DNL Users in This Role]列标题下，您可以选择单击用户的超链接电子邮件地址。 此时会为特定用户打开[!DNL Role Membership]页面，您可以在该页面中将用户分配到角色，也可以从角色中取消分配用户。

请参阅[配置角色成员关系](../c-about-settings-menu/c-about-users-menu.md#task_DAC596AAFFCE4EF0BE68CEEF7E365414)。

在表的[!DNL Roles]列标题下，您可以选择单击超链接的角色名称。 此时将打开[!DNL Role Membership]页面，您可以在其中将用户分配给选定的角色，也可以从选定的角色中取消分配用户。
...
## 配置角色成员资格{#task_DAC596AAFFCE4EF0BE68CEEF7E365414}

您可以使用[!DNL Role Membership]页面向角色添加用户或从角色中删除用户。
...
1. 在[!DNL Role Membership]页面上，执行下列操作之一：
   ...
   <table> 
      ...
      <td class="choption"><strong>向单个角色添加一个或多个用户</strong></td> 
      <td class="chdesc stentry"> <p>
      <ul id="ul_59E7C36210804EF9B6A2706A5357A892"> 
      <li id="li_2A8D31C968B543EBA7948DD4EFA350AA"> 在<span class="uicontrol">更改角色</span>下拉列表中，选择要向其添加用户的角色。 <p>如果未显示<span class="uicontrol">更改角色</span>下拉列表，请单击<span class="uicontrol">视图用户BY GROUP</span>。 </p> </li> 
      <li id="li_3A67F0DDBDBE4883A17300A3F088D71A"> （可选）在表中，选中<span class="uicontrol">仅显示角色成员</span>，使表仅显示当前分配给选定角色的帐户用户。 </li> 
      ...
      <td class="chdesc stentry"> <p> 
         ...
         <li id="li_7830E87D6924433DBBA03C953B9452A2"> （可选）在表中，选中<span class="uicontrol">仅显示此用户的角色</span>，使表仅显示当前分配给选定帐户用户的那些角色。 </li> 
         <li id="li_DE742B95BFC34BF4AE338B165B533FDF"> 在表的复选框列中，选择要分配给选定用户的一个或多个角色。 <p>取消选择要从选定用户删除的一个或多个角色。 </p> </li> 
         </ul> </p> </td> 
      </tr> 
   </table>
